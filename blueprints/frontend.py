...
async def settings_banner():
    # if not authenticated; render login
    if not 'authenticated' in session:
        return await flash('error', 'You must be logged in to access profile banner settings!', 'login')
...
async def settings_banner_post():
    ...
    # no file uploaded; deny post
    if not banner or filename == '':
        return await flash('error', 'No image was selected!', 'settings/banner')

    # bad file extension; deny post
    if not file_extension in ALLOWED_EXTENSIONS:
        return await flash('error', 'The image you select must be either a .JPG, .JPEG, or .PNG file!', 'settings/banner')

    # remove old avatars
    tasks = [aos.remove(f'{BANNERS_PATH}/{session["user_data"]["id"]}{fx}') for fx in ALLOWED_EXTENSIONS]
    await asyncio.gather(*tasks, return_exceptions=True)

    # avatar change success
    banner.save(os.path.join(BANNERS_PATH, f'{session["user_data"]["id"]}{file_extension.lower()}'))
    return await flash('success', 'Your profile banner has been successfully changed!', 'settings/banner')
